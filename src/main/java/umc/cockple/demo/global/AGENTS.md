...
| Realtime routing | `realtime/routing/` | Domain/action route registry and the handler SPI implemented by feature slices |
...
- Realtime feature slices implement `RealtimeDomainHandler`; the shared router must not branch on feature names.
